...
class FPM 
{
    ...
    bool readDataPacket(uint8_t * destBuffer, IFpmStream * destStream, uint16_t * readLen, bool * readComplete);
    bool writeDataPacket(uint8_t * srcBuffer, IFpmStream * srcStream, uint16_t * writeLen, bool writeComplete);
    ...
    FPMStatus getLastIndex(int16_t * lastIndex);
     ...
     */
    FPMStatus writePacket(uint8_t * srcBuffer, IFpmStream * srcStream, uint16_t * writeLen, uint8_t pktId);
    /**
     ...
     *   @return                    If successful, LIB_OK. Else, an error code.
     */
    FPMStatus readPacket(uint8_t * destBuffer, IFpmStream * destStream, uint16_t * readLen, uint8_t * pktId); 
