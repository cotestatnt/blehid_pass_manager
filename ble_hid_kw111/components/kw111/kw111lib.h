...
#define DEFAULTTIMEOUT 100000 //!< UART reading timeout in microseconds
...
{
public:
  FingerprintSensor(uart_port_t uartNum, int rxPin, int txPin, uint32_t address, uint32_t password);
  virtual ~FingerprintSensor();

  uint8_t begin(uint16_t baudrate);
  uint8_t readDeviceInfo(FingerprintDeviceInfo &info);
  ...
  uint8_t getStructuredPacket(Fingerprint_Packet *p, uint32_t timeout = DEFAULTTIMEOUT);
...
private:
  ...
  uint8_t getCmdPacket(Fingerprint_Packet &packet, uint8_t dataLen, ...);
  uint8_t sendCmdPacket(uint8_t dataLen, ...);
