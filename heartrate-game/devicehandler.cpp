...
void DeviceHandler::updateCharacteristicValue(const QLowEnergyCharacteristic &c, const QByteArray &value)
{
    ...
    if (data.isEmpty() || data.at(0) == '\0') {
        ...
        qDebug() << "[BLE Notify] Lista utenti completata." << list;
        emit userListUpdated();
        return;
    }
...
//     if (data.isEmpty() || data.at(0) == '\0') {
//         qDebug() << "[BLE Notify] Fine lista utenti.";
//         m_readingUsers = false;
//         emit userListUpdated();
//         return;
//     }
...
//     // Richiesta automatica del prossimo utente
//     if (m_readingUsers && cmd == 0x04) {
//         // Dopo username, chiediamo la password per lo stesso indice
//         requestNextUser(0x05);
//     } else if (m_readingUsers && cmd == 0x05) {
//         // Dopo password, passiamo al prossimo indice
//         m_currentUserIndex++;
//         requestNextUser(0x04);
//     }
// }
