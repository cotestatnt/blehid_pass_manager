// R503Lib.cpp adattato per ESP-IDF
 ...
 */
R503Lib::R503Lib(uart_port_t uartNum, int rxPin, int txPin, uint32_t address)
    : uart_num(uartNum), fpsRxPin(rxPin), fpsTxPin(txPin), fpsAddress(address) {    
}
...
uint8_t R503Lib::begin(long baudrate, uint32_t passwd) {
    ...
    if (retVal != R503_OK) {
        ESP_LOGE(TAG, "readParameters failed: 0x%02X", retVal);
        return retVal;
    }
    ...
    if (retVal != R503_OK) {
        ESP_LOGE(TAG, "readDeviceInfo failed: 0x%02X", retVal);
        return retVal;
    }
...
uint8_t R503Lib::setBaudrate(long baudrate) {
    switch (baudrate) {
        ...
        case 57600:
        case 115200: {
            uint8_t value = static_cast<uint8_t>(baudrate / 9600);
            uint8_t confirmationCode = writeParameter(4, value);
...
uint8_t R503Lib::setPacketSize(uint16_t size) {
    uint8_t value = 2; // Default to 128 bytes
    ...
    switch (size) {
        ...
        default:
            ESP_LOGE(TAG, "invalid packet size: %d", size);
...
uint8_t R503Lib::softReset() {
    // Send soft reset command (0x0D)
   uint8_t payload[] = {
        static_cast<uint8_t>((fpsPasswd >> 24) & 0xFF),
        static_cast<uint8_t>((fpsPasswd >> 16) & 0xFF),
        static_cast<uint8_t>((fpsPasswd >> 8) & 0xFF),
        static_cast<uint8_t>(fpsPasswd & 0xFF),
        0x0D
    };

    R503Packet packet(sizeof(payload), R503_PKT_COMMAND, payload);
    sendPacket(packet);

    uint8_t response[12];
    uint16_t length = sizeof(response);
    return receiveAck(response, length);
}
/* --------------------------
    ? Fingerprint Related
----------------------------*/
...
void R503Lib::sendPacket(const R503Packet &packet) {
    ...
    uint8_t header[] = {
        static_cast<uint8_t>(R503_PKT_START_CODE >> 8), static_cast<uint8_t>(R503_PKT_START_CODE & 0xFF),
        static_cast<uint8_t>(fpsAddress >> 24), static_cast<uint8_t>(fpsAddress >> 16),
        static_cast<uint8_t>(fpsAddress >> 8), static_cast<uint8_t>(fpsAddress),
        packet.type,
        static_cast<uint8_t>(length >> 8), static_cast<uint8_t>(length & 0xFF)
    };

    uart_write_bytes(uart_num, (const char *)header, sizeof(header));
    uart_write_bytes(uart_num, (const char *)packet.payload, packet.length);
    uint8_t checksum[2] = {
        static_cast<uint8_t>(packet.checksum >> 8),
        static_cast<uint8_t>(packet.checksum & 0xFF)
    };
    uart_write_bytes(uart_num, (const char *)checksum, 2);

#if R503_DEBUG
    ESP_LOGD(TAG, "\n>> Sent packet:");
    ESP_LOGD(TAG, "- startCode: %02X %02X", header[0], header[1]);
    ESP_LOGD(TAG, "- address: %02X %02X %02X %02X", header[2], header[3], header[4], header[5]);
    ESP_LOGD(TAG, "- type: %02X", packet.type);
    ESP_LOGD(TAG, "- length: %02X %02X (%d bytes inc. checksum)", header[7], header[8], length);

    std::string payload_str;
    for (int i = 0; i < packet.length; ++i) {
        char byte_str[4];
        snprintf(byte_str, sizeof(byte_str), "%02X ", packet.payload[i]);
        payload_str += byte_str;
    }
    ESP_LOGD(TAG, "- payload: %s", payload_str.c_str());
    ESP_LOGD(TAG, "- checksum: %02X %02X", checksum[0], checksum[1]);
    ESP_LOGD(TAG, "-------------------------");
#else
    ESP_LOGD(TAG, "Sent packet: type=0x%02X, length=%d", packet.type, packet.length);
#endif
}
...
uint8_t R503Lib::receivePacket(R503Packet &packet) {
    ...
    // Wait for start code (0xEF)
    while ((esp_timer_get_time() - start_time) < R503_RECEIVE_TIMEOUT * 1000) {
        uint8_t byte;
    ...
    if ((esp_timer_get_time() - start_time) >= R503_RECEIVE_TIMEOUT * 1000) {
        ESP_LOGE(TAG, "Timeout waiting for start code");
        return R503_TIMEOUT;
    }

    // Read remaining header (address, type, length)
    int read_len = uart_read_bytes(uart_num, buffer + 1, 8, 100 / portTICK_PERIOD_MS);
    if (read_len < 8) {
        ESP_LOGE(TAG, "Incomplete packet header");
        return R503_TIMEOUT;
    }
    ...
    if (startCode != R503_PKT_START_CODE) {
        ESP_LOGE(TAG, "Invalid start code: %02X %02X", buffer[0], buffer[1]);
    ...
    uart_read_bytes(uart_num, packet.payload, packet.length, 100 / portTICK_PERIOD_MS);

    uint8_t checksumBuffer[2];
    uart_read_bytes(uart_num, checksumBuffer, 2, 100 / portTICK_PERIOD_MS);
    packet.checksum = (checksumBuffer[0] << 8) | checksumBuffer[1];

    // Debug output for received packet
    ESP_LOGD(TAG, "\n>> Received packet:");
    ESP_LOGD(TAG, "- startCode: %02X %02X", buffer[0], buffer[1]);
    ESP_LOGD(TAG, "- address: %02X %02X %02X %02X", buffer[2], buffer[3], buffer[4], buffer[5]);
    ESP_LOGD(TAG, "- type: %02X", packet.type);
    ESP_LOGD(TAG, "- length: %02X %02X (%d bytes inc. checksum)", buffer[7], buffer[8], packet.length + 2);
    ...
    ESP_LOGD(TAG, "- payload: %s", payload_str.c_str());
    ESP_LOGD(TAG, "- checksum: %02X %02X", checksumBuffer[0], checksumBuffer[1]);
...
uint8_t R503Lib::receiveData(uint8_t *data, uint16_t &length) {
    ...
    // Read packets until 0xEF01 ... 08 (DATA_END) is found
    while ((esp_timer_get_time() - start_time) < 4000000) { // 4s timeout
        uint8_t byte;
        int len = uart_read_bytes(uart_num, &byte, 1, 10 / portTICK_PERIOD_MS);
        if (len > 0) {
            tempBuffer[tempIndex++] = byte;
            if (tempIndex >= 8 && tempBuffer[tempIndex - 8] == 0xEF && tempBuffer[tempIndex - 7] == 0x01 && tempBuffer[tempIndex - 2] == 0x08) {
                break;
            }
        }
    }
    ...
    for (uint32_t i = 0; i < tempIndex; i++) {  
        ESP_LOGD(TAG, "%02X ", tempBuffer[i]);
    }   
#else
    ESP_LOGD(TAG, "Received %d bytes of data", length);
#endif

    if (tempIndex < 9) return R503_TIMEOUT;

    // Extract data from packets, skipping headers and checksums
    uint32_t outputIndex = 0;
    for (uint32_t i = 0; i < tempIndex - 1; i++) {
        if (tempBuffer[i] == 0xEF && tempBuffer[i + 1] == 0x01) {
            uint32_t start = i + 9;
            if (start >= tempIndex) break;
            uint16_t dataLen = ((tempBuffer[i + 7] << 8) | tempBuffer[i + 8]) - 2;
            if (start + dataLen > tempIndex) break;
            memcpy(&data[outputIndex], &tempBuffer[start], dataLen);
            outputIndex += dataLen;
            i = start + dataLen + 1; // Skip checksum too
        }
    }
...
uint8_t R503Lib::printParameters() {
    ...
    if (retVal == R503_OK) {
        ESP_LOGI(TAG, "Status Register: 0x%04X", params.statusRegister);
        ESP_LOGI(TAG, "System Identifier Code: 0x%04X", params.systemIdentifierCode);
        ESP_LOGI(TAG, "Finger Library Capacity: %u", params.fingerLibrarySize);
        ESP_LOGI(TAG, "Security Level: %u", params.securityLevel);
        ESP_LOGI(TAG, "Device Address: 0x%08lX", params.deviceAddress);
        ESP_LOGI(TAG, "Data Package Size: %u bytes", params.dataPackageSize);
        ESP_LOGI(TAG, "Baudrate: %lu", params.baudrate);
    } else {
        ESP_LOGE(TAG, "Error retrieving parameters (code: 0x%02X)", retVal);
    }
    return retVal;
}
